...
- [`npi_summarize()`](https://docs.ropensci.org/npi/reference/npi_summarize.md):
  A method for displaying a nice overview of results from
  [`npi_search()`](https://docs.ropensci.org/npi/reference/npi_search.md).
- [`npi_flatten()`](https://docs.ropensci.org/npi/reference/npi_flatten.md):
  A method for flattening one or more list columns from a search result,
  joined by NPI number.
- [`npi_is_valid()`](https://docs.ropensci.org/npi/reference/npi_is_valid.md):
  Check the validity of one or more NPI numbers using the official NPI
...
Run
[`npi_summarize()`](https://docs.ropensci.org/npi/reference/npi_summarize.md)
on your results to see a more human-readable overview of your search
results. Specifically, the function returns the NPI number, provider’s
name, enumeration type (individual or organizational provider), primary
address, phone number, and primary taxonomy (area of practice). If no
taxonomy is marked as primary for a record, the first taxonomy listed
for that NPI is returned:
...
[`npi_search()`](https://docs.ropensci.org/npi/reference/npi_search.md)
has a nested structure. Although all the data in a single row relates to
one NPI, each list column contains a list of one or more values
corresponding to the NPI for that row. For example, a provider’s NPI
record may have multiple associated addresses, phone numbers,
taxonomies, and other attributes, all of which live in the same row of
the data frame.

Because nested structures can be a little tricky to work with, the `npi`
includes
[`npi_flatten()`](https://docs.ropensci.org/npi/reference/npi_flatten.md),
a function that transforms the data frame into a flatter (i.e., unnested
and merged) structure that’s easier to use.
[`npi_flatten()`](https://docs.ropensci.org/npi/reference/npi_flatten.md)
...
[`npi_flatten()`](https://docs.ropensci.org/npi/reference/npi_flatten.md)
supports a variety of approaches to flattening the results from
[`npi_search()`](https://docs.ropensci.org/npi/reference/npi_search.md).
...
purposes. Use
[`npi_is_valid()`](https://docs.ropensci.org/npi/reference/npi_is_valid.md)
to check whether an NPI number you’ve encountered is validly
constructed:
