...
- [`npi_search()`](https://docs.ropensci.org/npi/reference/npi_search.md)
  retains previously retrieved records when a later page is empty.
- [`npi_flatten()`](https://docs.ropensci.org/npi/reference/npi_flatten.md)
  preserves all input keys when selected nested data is empty,
  regardless of column order.
- [`npi_summarize()`](https://docs.ropensci.org/npi/reference/npi_summarize.md)
  formats complete addresses when an optional second address line is
  absent or missing.
...
- [`npi_search()`](https://docs.ropensci.org/npi/reference/npi_search.md)
  now normalizes `address_purpose` values to API constants and accepts
  case-insensitive input.
- [`npi_is_valid()`](https://docs.ropensci.org/npi/reference/npi_is_valid.md)
  now supports vector inputs and returns logical vectors.
- [`npi_search()`](https://docs.ropensci.org/npi/reference/npi_search.md)
  returns a typed empty `npi_results` object when no records are found.

### BUG FIXES

- [`npi_summarize()`](https://docs.ropensci.org/npi/reference/npi_summarize.md)
  now preserves input rows when NPI records have empty or missing nested
  address or taxonomy data.
...
- Clarified
  [`npi_summarize()`](https://docs.ropensci.org/npi/reference/npi_summarize.md)
  taxonomy fallback behavior when no primary taxonomy is marked.
...
- [`npi_flatten()`](https://docs.ropensci.org/npi/reference/npi_flatten.md)
  ...
  ([\#15](https://github.com/ropensci/npi/issues/15) and
  [\#73](https://github.com/ropensci/npi/issues/73)).
- [`npi_search()`](https://docs.ropensci.org/npi/reference/npi_search.md)
  provides an informative error message when internet is off or the
  endpoint is unreachable
  ([\#52](https://github.com/ropensci/npi/issues/52)).
- [`npi_search()`](https://docs.ropensci.org/npi/reference/npi_search.md)
  now checks for legal use of special characters prior to submitting an
  API request ([\#53](https://github.com/ropensci/npi/issues/53)).
- [`npi_search()`](https://docs.ropensci.org/npi/reference/npi_search.md)
  now checks for legal use of wildcard characters prior to submitting an
  API request ([\#54](https://github.com/ropensci/npi/issues/54)).
...
- [`npi_search()`](https://docs.ropensci.org/npi/reference/npi_search.md)
  now has test coverage for the case where empty results are returned
  ([\#59](https://github.com/ropensci/npi/issues/59)).
- [`npi_search()`](https://docs.ropensci.org/npi/reference/npi_search.md)
  now provides a message indicating how many records are being requested
  ([\#72](https://github.com/ropensci/npi/issues/72)).
- [`npi_search()`](https://docs.ropensci.org/npi/reference/npi_search.md)
  now handles epoch field conversion to date format, fixing a breaking
  API change. Thanks to [@trang-n](https://github.com/trang-n) for
...
- [`npi_summarize()`](https://docs.ropensci.org/npi/reference/npi_summarize.md)
  now handles NPI records in which all provider taxonomy records have
  been flagged as non-primary
  ([\#51](https://github.com/ropensci/npi/issues/51)).
...
- [`npi_search()`](https://docs.ropensci.org/npi/reference/npi_search.md)
  documentation now links to the NPPES data dictionary to help users
  understand the API results
...
- [`npi_is_valid()`](https://docs.ropensci.org/npi/reference/npi_is_valid.md)
  now uses the checkLuhn package for Luhn checking
